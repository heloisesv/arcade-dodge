...
/* Effet scanlines / glow pour le côté arcade */
.scanlines {
  pointer-events: none;
  position: fixed;
  inset: 0;
...
.glow-overlay {
  ...
  background:
    ...
    radial-gradient(circle at 80% 80%, rgba(244, 63, 94, 0.2), transparent 60%);
  z-index: 0;
}
...
#game-area {
  ...
  background:
    ...
    radial-gradient(circle at 90% 100%, rgba(244, 63, 94, 0.2), transparent 60%),
    #020617;
  position: relative;
  overflow: hidden;
  box-shadow:
...
#player {
  ...
  height: 26px;
  border-radius: 50%;
  background: radial-gradient(circle at 30% 30%, #f9fafb 0%, #22d3ee 40%, #22c55e 80%);
  box-shadow:
...
.enemy {
  position: absolute;
  border-radius: 50%;
  background: radial-gradient(circle at 30% 30%, #f9fafb 0%, #f97316 30%, #ef4444 90%);
  box-shadow:
...
@media (max-width: 700px) {
  ...
  #game-area {
    height: 70vh; /* occupe bien l'écran */
  }
}
